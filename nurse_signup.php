<?php
	...
	if($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if(!empty($firstName) && !empty($lastName) && !empty($birthday) && !empty($age) && !empty($nurseNumber) && !empty($username) && !empty($email) && !empty($phoneNumber) && !empty($password) && !is_numeric($firstName) && !is_numeric($lastName) && !is_numeric($usename) && !is_numeric($email)) {

			//save to database
			$user_id = random_num(20);
			$query = "insert into nurse (user_id,type,firstName,lastName,birthday,age,sex,nurseNumber,username,email,phoneNumber,password) values ('$user_id','Nurse','$firstName','$lastName','$birthday','$age','$sex','$nurseNumber','$username','$email','$phoneNumber','$password')";
