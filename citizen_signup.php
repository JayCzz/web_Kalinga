<?php
	...
	if($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if(!empty($firstName) && !empty($lastName) && !empty($birthday) && !empty($age) && !empty($familyNumber) && !empty($username) && !empty($email) && !empty($phoneNumber) && !empty($password) && !is_numeric($firstName) && !is_numeric($lastName) && !is_numeric($usename) && !is_numeric($email)) {
			...
			$query = "insert into users (user_id,type,firstName,lastName,birthday,age,sex,familyNumber,username,email,phoneNumber,password) values ('$user_id','Citizen','$firstName','$lastName','$birthday','$age','$sex','$familyNumber','$username','$email','$phoneNumber','$password')";
